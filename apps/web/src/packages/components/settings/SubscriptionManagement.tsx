...
export function SubscriptionManagement({ teamId, onUpgrade }: SubscriptionManagementProps) {
	...
	const effectiveTeamId = teamId || subscription?.teamId || teams[0]?.id
	...
	const handleSelectPlan = async (planId: string) => {
		...
		// For plan change, we need existing subscription
		if (!effectiveTeamId && !subscription) {
		...
		try {
			...
			if (subscription) {
				...
				// Use existing subscription ID for payment
			...
			if (subscriptionId) {
				...
				toast('Не удалось создать подписку. Попробуйте снова.', 'error')
			}
		} catch (e: any) {
						...
						>
							<AnimatePresence mode="popLayout">
								{plans.map((plan, index) => {
									const isCurrent = subscription?.planRef?.id === plan.id
									...
									<CardContent className="p-5 flex flex-col h-full relative z-0 min-h-[600px]">
										...
										{/* Pricing Section */}
										<motion.div 
											className="mb-3 pb-3 border-b border-border/50 flex-shrink-0"
											initial={{ opacity: 0, y: 10 }}
											animate={{ opacity: 1, y: 0 }}
										...
										{/* Limits - Compact Grid */}
										<motion.div 
											className="grid grid-cols-1 gap-1.5 mb-3 flex-shrink-0"
											initial={{ opacity: 0 }}
											animate={{ opacity: 1 }}
										...
										<div className="flex flex-col flex-1 min-h-0">
											{plan.features.length > 0 && (() => {
												...
												return (
													<motion.div 
														className="pt-3 border-t border-border/50 flex-1 min-h-0 flex flex-col"
														initial={{ opacity: 0 }}
														animate={{ opacity: 1 }}
														transition={{ delay: 0.5 }}
													>
										...
										{/* Action Buttons */}
										<motion.div 
											className="mt-auto pt-3 border-t border-border/50 space-y-2 flex-shrink-0"
											initial={{ opacity: 0, y: 10 }}
											animate={{ opacity: 1, y: 0 }}
										...
										>
											...
											{isCurrent ? (
												<Button 
													variant="outline" 
													className="w-full border-primary/50 bg-primary/5 hover:bg-primary/10" 
													disabled
												>
			...
			<Dialog open={showProviderSelector} onOpenChange={setShowProviderSelector}>
				<DialogContent className="sm:max-w-md">
					<DialogHeader>
						...
						<DialogDescription>
							{subscription
								? 'Выберите платёжную систему для оплаты нового тарифа'
								: 'Выберите платёжную систему для оплаты подписки'}
						</DialogDescription>
					</DialogHeader>
					...
					<DialogFooter className="flex-col sm:flex-row gap-2">
						...
						<Button
							...
							disabled={!selectedProvider || creating || changing}
							className="w-full sm:w-auto"
						>
							{creating || changing ? (
								<>
