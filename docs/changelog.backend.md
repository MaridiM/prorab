...
### Added
- **Delete Project**: Реализована мутация `deleteProject`
  - Каскадное удаление всех связанных задач, столбцов и файлов
  - Проверка прав доступа (только владелец команды)

## [1.8.1] - 2026-01-11 - Global Subscription Limits 🔧

### Fixed
- **Global Limits**: Лимиты проектов и участников теперь считаются глобально по всем командам владельца
- **Dynamic Limits**: Лимит участников масштабируется от количества проектов (например, 2 проекта = 6 участников)
- **Owner Exclusion**: Владелец команды больше не учитывается в лимите участников (free seat)
...
### Fixed
- **Payment Mock**: Исправлена логика подтверждения тестовых платежей (mock) для обработки крайних случаев
