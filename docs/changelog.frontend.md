...
### Added
- **Project Actions**: Добавлена возможность удаления проектов (с подтверждением через AlertDialog)
- **Archive Functionality**: Добавлены действия архивации и восстановления проектов в карточки
- **Dashboard**: Полнофункциональная карточка проекта на главном дашборде
  - Меню действий ("More" button) с опциями Edit, Archive, Delete
  - Улучшенный UI без перекрытий

### Fixed
- **Event Propagation**: Исправлена проблема, когда клик на меню действий открывал страницу проекта
- **UI Layout**: Исправлено перекрытие бейджа статуса кнопкой действий

## [1.8.1] - 2026-01-11 - Global Subscription & UI Fixes 🔧

### Changed
- **Footer**: Обновлен текст "About App" (region-neutral)

## [1.8.0] - 2026-01-07 - Onboarding Redesign & Checkout 🚀

### Added
- **Payment Checkout Integration**: Интеграция процесса оплаты в онбординг (Step 4)
  - Автоматический редирект на платежный шлюз при выборе платного тарифа
  - Поддержка статуса PENDING_PAYMENT для новых подписок
- **Accordion Features**: Раскрывающиеся списки возможностей тарифов
  - Изначально показаны 5 ключевых функций
  - Кнопка "Показать все" для полного списка
- **Onboarding Persistence**: Сохранение прогресса онбординга
  - Переход с sessionStorage на localStorage для надежности
  - Данные сохраняются при перезагрузке страницы (F5)
  - Автоматическая очистка после успешного завершения

### Changed
- **Onboarding UI Redesign**: Полный редизайн шага выбора тарифа
  - Визуальное соответствие компоненту SubscriptionManagement
  - Убрано ограничение ширины (max-width) для лучшего отображения 3 карточек
  - Компактный и современный стиль карточек
- **Navigation Updates**:
  - Кнопка "Назад" перемещена в левую часть заголовка (выравнивание по левому краю контента)
  - Убрана кнопка "Завершить настройку" (автоматическое завершение при выборе Free)
  - Центрирование заголовка "Выберите тарифный план" независимо от кнопок навигации

### Fixed
- **Reload Data Loss**: Исправлена проблема сброса онбординга в начало при перезагрузке страницы
- **Layout Constraints**: Исправлена проблема узких карточек тарифов на больших экранах

## [1.7.1] - 2026-01-06 - Индикация Тестового Периода 🎯

### Added
- **Trial Period Indicators**: Добавлена визуальная индикация тестового периода
  - Бейдж "X дней бесплатно" на карточках планов в списке тарифов
  - Бейдж "Пробный период" на странице текущей подписки
  - Индикация оставшихся дней тестового периода в шапке (UpgradeButton)
  - Отображение количества дней с правильным склонением (день/дня/дней)

### Changed
- **UpgradeButton Component**: Улучшен компонент кнопки апгрейда в шапке
  - Добавлен бейдж с оставшимися днями тестового периода
  - Отображается только для подписок со статусом TRIALING
  - Правильное склонение дней на русском языке
  - Иконка Clock для визуального выделения

- **SubscriptionManagement Component**: Улучшено отображение тестового периода
  - Бейдж "Пробный период" добавлен рядом со статусом подписки
  - Индикация тестового периода на карточках планов в списке
  - Визуальное выделение планов с доступным тестовым периодом

### Fixed
- **Onboarding Recovery**: Исправлена ошибка "Onboarding already completed"
  - Пользователи могут продолжить создание команды, если онбординг отмечен как завершенный, но команда не создана
- **SubscriptionHistory Component**: Исправлена ошибка с порядком хуков React
  - Перемещен `useState` для пагинации в начало компонента
  - Все хуки теперь вызываются до условных возвратов
  - Исправлена ошибка "Rendered more hooks than during the previous render"

- **Auth Context**: Улучшена обработка случая `me === null`
  - Добавлена явная проверка `data?.me === null` в `refreshUser`
  - Корректный редирект на страницу авторизации при отсутствии пользователя
  - Предотвращение бесконечного loading при неавторизованном доступе

### Changed
- **Payment Success Page**: Полный редизайн страницы успешной оплаты
  - Добавлены конфетти и анимации
  - Премиум UI компоненты и улучшенная навигация
- **UpgradeButton Component**: Улучшен компонент кнопки апгрейда в шапке
  - Добавлен бейдж с оставшимися днями тестового периода
  - Отображается только для подписок со статусом TRIALING
  - Правильное склонение дней на русском языке
...
### Fixed
- **Seed Users Roles**: Улучшена логика обновления ролей и прав доступа для seed пользователей
  - Теперь роли и права доступа обновляются для существующих пользователей, если они не соответствуют seed данным
  - Добавлена проверка соответствия прав доступа для каждой роли
  - Если у пользователя есть роль, которой не должно быть (согласно seed), она удаляется
  - Для новых пользователей роли создаются с правильными правами доступа

### Changed
- Обновлена логика seed файла для проверки и обновления ролей существующих пользователей
- Добавлено логирование количества прав доступа при создании/обновлении роли
- Улучшена проверка соответствия ролей и прав доступа

## [1.6.20] - 2026-01-02 - Database Reset & Period Tracking Fix

### Fixed
- **Payment Period Tracking**: Исправлена логика сохранения `period_start_at` и `period_end_at` при оплате
  - Добавлено логирование для отладки сохранения периодов
  - Периоды теперь корректно сохраняются при каждой оплате подписки
  - Используется `paidAt` для консистентности данных
- **Subscription History Period Display**: Исправлено отображение периодов в истории подписок
  - Для текущего плана используются данные из `subscriptions.currentPeriodStart/End`
  - Для исторических платежей используются данные из `payments.periodStartAt/End`
- **Database Reset Script**: Создан скрипт для полного сброса базы данных
  - Скрипт проверяет существование таблиц перед удалением
  - Сохраняет зарезервированные таблицы (планы, провайдеры, системные настройки)
  - Добавлена команда `pnpm db:reset` для сброса базы данных
  - Добавлена команда `pnpm db:reset:seed` для сброса и повторного заполнения
- **Prisma Schema**: Исправлена конфигурация для Prisma 7
  - Удалено поле `url` из `datasource` (не поддерживается в Prisma 7)
  - Исправлен `DATABASE_URL` в `.env` (используется прямая строка подключения)

### Added
- **Database Reset Scripts**: Добавлены скрипты для управления базой данных
  - `scripts/reset-database.ts` - скрипт для сброса базы данных
  - Команда `db:reset` в package.json
  - Команда `db:reset:seed` для полного сброса и повторного заполнения
- **Period Fields Migration**: Добавлена миграция для полей периодов в таблице `payments`
  - Поля `period_start_at` и `period_end_at` добавлены в таблицу `payments`
  - Существующие платежи обновлены с периодными данными

### Changed
- Улучшена логика сохранения периодов в `payments.service.ts`
- Добавлено логирование для отладки сохранения периодов
- Обновлена логика resolver для использования данных из подписки для текущего плана
- Subscription history now correctly displays period dates from database for all payments
- Current plan period dates are now sourced from subscription table for accuracy

## [1.6.19] - 2026-01-02 - Subscription Period Fixes & History Improvements

### Fixed
- **Subscription Period Calculation**: Исправлен расчет периодов подписки
  - Периоды теперь рассчитываются от даты оплаты, а не от будущих дат
  - Для текущего плана используются периоды из базы данных (сохраняются при оплате)
  - Для исторических платежей периоды рассчитываются от даты оплаты + 30 дней
  - При продлении плана период начинается с даты оплаты, а не с конца предыдущего периода
  - Убрана сложная логика с цепочками платежей, которая приводила к неправильным датам
- **Subscription History Renewal Logic**: Исправлена логика определения продлений в истории подписок
  - Теперь правильно определяется, был ли платеж продлением того же плана или сменой плана
  - Если предыдущий платеж (более старый) - тот же план, то текущий платеж помечается как "Продлена"
  - Если предыдущий платеж - другой план, то текущий платеж - это смена плана, и предыдущий план помечается как "Подписка завершена"
  - Пример: если перешли с Лайт на Прораб, то Лайт будет "завершен", а не "продлен"
  - Пример: если продлили Прораб, то предыдущий платеж Прораб будет "продлен"
